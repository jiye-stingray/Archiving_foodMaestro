...
Unity와 C#으로 개발한 1인 제작 모바일 레스토랑 경영 시뮬레이션 게임입니다. 손님 응대, 요리, 직원 관리, 주방 업그레이드 등 식당 운영의 핵심 루프를 구현하였으며, Easy Save 3 기반의 로컬 저장 시스템과 DOTween·PolyNav 등 외부 플러그인을 통합하여 완성도 높은 게임플레이를 구성했습니다. 단순한 기능 구현을 넘어 Singleton, FSM, ScriptableObject, Object Pooling 등 실전적인 아키텍처 패턴을 적용하여 유지보수성과 확장성을 갖춘 구조를 설계하는 것을 목표로 했습니다.
...
### 4. Room Customization & Grid System (격자 기반 가구 배치)

플레이어가 레스토랑 인테리어를 자유롭게 꾸밀 수 있는 룸 커스터마이징 씬을 구현했습니다. 가구를 격자 위에 배치·이동·제거하고, 배치 상태를 저장하여 씬 재진입 시 복원해야 했습니다.

**구현 내용**
- `GridManager`로 타일 단위 격자 좌표 계산 및 점유 상태 관리
- `FurnitureManager`에서 가구 인벤토리, 배치 가능 여부 체크, 배치 확정 처리
- `Furniture` ScriptableObject로 가구 이름·인덱스·스프라이트 데이터 정의
- EnhancedScroller v2로 가구 목록 UI 가상화 스크롤 적용
- 배치 결과를 Easy Save 3로 저장하여 씬 재진입 시 자동 복원

**배운 점**
- 격자 좌표계와 월드 좌표계 사이의 변환 로직을 명확히 분리하면 배치 정확도 디버깅이 쉬워짐을 경험
- UI 가상화(EnhancedScroller)가 대량 아이템 목록에서 드로우콜과 메모리를 크게 줄임을 실측으로 확인

<br/>

### 5. Easy Save 3 Save System (로컬 저장 시스템)
...
### 7. Multi-Stage Progression & Scene Management (스테이지 진행 및 씬 전환)
